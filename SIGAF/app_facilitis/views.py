...
def cadastro(request):
    if request.method== 'POST':
        ...
        local = Cadastro.objects.create(nome_do_espaco = nome_do_espaco,tamanho_do_espaco = tamanho_do_espaco,
...
def editar_local(request, local_id):
    ...
    return render(request, 'facilitis/editar_local.html', editar_cadastro)

def atualiza_local(request):
    if request.method == 'POST':
        local_id = request.POST['id']
        local = Cadastro.objects.get(pk=local_id)
